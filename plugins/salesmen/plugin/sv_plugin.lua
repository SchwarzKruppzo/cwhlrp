...
Clockwork.datastream:Hook("SalesmanAdd", function(player, data)
	if (player.cwSalesmanSetup) then
		...
		salesman:Spawn();
		salesman:SetSkin(tonumber(data.skin));
		salesman:SetBodyGroups(data.bgroups);
		salesman.cwStock = {};
...
local function GetBGroups(entity)
	local groups = entity:GetBodyGroups();
	local bodygroups = "";

	for k, v in pairs(groups) do
		bodygroups = bodygroups .. entity:GetBodygroup(v.id);
	end;
